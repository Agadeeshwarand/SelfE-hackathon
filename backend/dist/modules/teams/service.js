...
async function listTeams(query) {
    ...
    const dir = String(query.dir ?? "desc") ===
        "asc"
        ? "asc"
        : "desc";
...
function buildTeamWhere(query) {
    ...
    if (query.eligible === "false" ||
        query.eligible === false) {
        where.isEligible = false;
    }
    const mentorAssigned = query.mentorAssigned ??
        query.assigned;
    if (mentorAssigned === "true" ||
        mentorAssigned === true) {
        where.guidanceAssignment = {
            isNot: null,
        };
    }
    if (mentorAssigned === "false" ||
        mentorAssigned === false) {
        where.guidanceAssignment = {
            is: null,
        };
    }
    const department = typeof query.department ===
        "string"
    ...
    const mentorId = typeof query.mentorId ===
        "string"
        ? query.mentorId.trim()
        : "";
    if (mentorId) {
...
async function removeMember(actor, teamId, studentId) {
    ...
    const actorStudent = await prisma_1.prisma.studentProfile.findUnique({
        where: {
            userId: actor.userId,
        },
    });
    const actorMembership = team.members.find((m) => m.studentId ===
        actorStudent?.id);
    const isAdmin = actor.role === "ADMIN";
    const isLeader = Boolean(actorMembership?.isLeader);
    const isSelf = actorStudent?.id ===
        studentId;
    if (!isAdmin &&
        !isLeader &&
        !isSelf) {
    ...
    if (target.isLeader &&
        ...
        throw new AppError_1.AppError(409, "Change the team leader before removing the current leader");
    }
    const targetStudent = await prisma_1.prisma.studentProfile.findUnique({
        where: {
            id: studentId,
        },
    });
...
async function updateTeam(teamId, input) {
    ...
    if (name !== undefined &&
        (name.length < 2 ||
            name.length > 80)) {
        throw new AppError_1.AppError(400, "Team name must be between 2 and 80 characters");
    }
    const leaderStudentId = input.leaderStudentId ===
        undefined ||
        input.leaderStudentId === null
        ? undefined
        : String(input.leaderStudentId);
    if (leaderStudentId !== undefined) {
        const newLeader = team.members.find((m) => m.studentId ===
            leaderStudentId);
        if (!newLeader) {
            throw new AppError_1.AppError(400, "The selected leader must be a member of this team");
        }
    }
    await prisma_1.prisma.$transaction(async (tx) => {
        if (name !== undefined) {
        ...
        if (leaderStudentId !==
            ...
            if (currentLeader?.id !==
                ...
                if (currentLeader) {
                    const oldLeader = await tx.studentProfile.findUniqueOrThrow({
                        where: {
                ...
                const newLeaderStudent = await tx.studentProfile.findUniqueOrThrow({
                    where: {
...
async function addMember(teamId, input) {
    const team = await prisma_1.prisma.team.findUnique({
        where: {
            id: teamId,
        },
    ...
    const studentId = typeof input.studentId ===
        "string"
        ? input.studentId.trim()
        : "";
    const registerNumber = typeof input.registerNumber ===
        "string"
        ? input.registerNumber.trim()
        : "";
    const student = studentId
        ? await prisma_1.prisma.studentProfile.findUnique({
            where: {
                id: studentId,
            },
        })
        : registerNumber
            ...
            : null;
    if (!student) {
        throw new AppError_1.AppError(404, "Registered student not found");
    }
    ...
    if (existingMembership) {
        throw new AppError_1.AppError(409, "This student already belongs to a team");
    }
    await prisma_1.prisma.teamMember.create({
        data: {
            studentId: student.id,

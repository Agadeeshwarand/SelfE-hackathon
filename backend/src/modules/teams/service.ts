...
import { serializeTeam, teamDetailInclude } from "./presenter";
...
async function getOwnStudentProfileOrThrow(userId: string) {
  const profile = await prisma.studentProfile.findUnique({
    where: { userId },
    include: { teamMembership: true },
  });
  if (!profile) {
    throw new AppError(403, "Only registered students can perform this action");
  }
  return profile;
}

async function assertStudentHasNoTeam(studentId: string) {
  const existing = await prisma.teamMember.findUnique({ where: { studentId } });
  if (existing) {
    throw new AppError(409, "You already belong to a team");
  }
}
...
function withEligibility(team: {
  ...
  createdAt: Date;
  updatedAt: Date;
  members: Parameters<typeof serializeTeam>[0]["members"];
  guidanceAssignment?: Parameters<typeof serializeTeam>[0]["guidanceAssignment"];
}) {
  const eligibility = evaluateTeamEligibility(
    team.members.map((m) => ({
      gender: m.student.gender as "MALE" | "FEMALE" | "OTHER" | "PREFER_NOT_TO_SAY",
      departmentId: m.student.departmentId,
    }))
  );
  return serializeTeam(team, eligibility);
}
...
export async function createTeam(userId: string, input: CreateTeamInput) {
  ...
  throw new AppError(500, "Could not generate a unique team code, please try again");
}

export async function joinTeam(userId: string, teamCode: string) {
  const student = await getOwnStudentProfileOrThrow(userId);
  await assertStudentHasNoTeam(student.id);
  ...
  if (!team) throw new AppError(404, "No team found with that code");
  if (team.members.length >= MAX_TEAM_SIZE) {
    throw new AppError(409, `Team is already full (${MAX_TEAM_SIZE}/${MAX_TEAM_SIZE})`);
  }
...
export async function getTeamWithEligibility(teamId: string) {
  return recalculateAndPersistEligibility(teamId);
}

export async function getMyTeam(userId: string) {
  const student = await getOwnStudentProfileOrThrow(userId);
  if (!student.teamMembership) return null;
  return getTeamWithEligibility(student.teamMembership.teamId);
}

export async function getTeamById(teamId: string, actor: { userId: string; role: string }) {
  const team = await prisma.team.findUnique({
    where: { id: teamId },
  ...
  if (actor.role === "MENTOR") {
    const mentor = await prisma.mentorProfile.findUnique({ where: { userId: actor.userId } });
    if (!mentor || team.guidanceAssignment?.mentorId !== mentor.id) {
      throw new AppError(403, "You do not have access to this team");
    }
    return withEligibility(team);
  }
...
export async function listTeams(query: Record<string, unknown>) {
  ...
  return paginated(
    rows.map((row) => withEligibility(row)),
    total,
    page,
    pageSize
  );
...
export function buildTeamWhere(query: Record<string, unknown>): Prisma.TeamWhereInput {
  ...
  if (query.eligible === "true" || query.eligible === true) where.isEligible = true;
  if (query.eligible === "false" || query.eligible === false) where.isEligible = false;
...
export async function removeMember(
  ...
  if (!target) throw new AppError(404, "Student is not a member of this team");

  const actorStudent = await prisma.studentProfile.findUnique({ where: { userId: actor.userId } });
  ...
  if (!isAdmin && !isLeader && !isSelf) {
    throw new AppError(403, "You cannot modify this team's membership");
  }

  if (target.isLeader && team.members.length > 1) {
    throw new AppError(409, "The team leader cannot leave while other members remain");
  }

  if (isSelf && target.isLeader && team.members.length > 1) {
    throw new AppError(409, "Transfer or remove members before leaving as team leader");
  }
...
async function recalculateAndPersistEligibility(teamId: string) {
  const team = await prisma.team.findUnique({
    where: { id: teamId },
    include: teamDetailInclude,
  });
  ...
  await prisma.team.update({
    ...
    data: {
      ...
      eligibilityDetails: eligibility as unknown as object,
    },
  });
